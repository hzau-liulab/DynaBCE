...
class RASA_feat(PDBfuc):
    def __init__(self, pdbfile=None, pc=None, max_asa=None, dsspexe=None, dssp_file=None):
        super().__init__(pdbfile)
...
class GE_feat(PDBfuc):
    def __init__(self, pdbfile=None, pc=None, ghecomexe=None, ghecomresfile=None):
        super().__init__(pdbfile)
    ...
    def ghecom(self, pc, pdbfile=None):
        ...
        """
        if pdbfile is not None:
            ge_out = f'./features/STR_feature/GE/{pc}-ghe.txt'
    ...
    def descriptor(self, ghecomresfile=None):
        ...
        return: str_array col.1=>res col.2=>shellAcc col.3=>Rinacc col.4=>pocketness
        """
        ghecomres = ghecomresfile if os.path.exists(ghecomresfile) else self.ghecom(self.pc, self.pdbfile)
...
def get_str_feat(args, pdbfile, pdb_fuc, pc):
    ...
    ghecom_feat = GE_feat(pdbfile, pc, args.ghecom)
