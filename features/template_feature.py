...
def topk_tm_search(pdbfile, pc, nw_cut=0.7, k=6):
...
def AlignmentMap(pdbfile, pdb_fuc, tm, tm_pc_feat, scaler):
...
def rotation_structure(pdbfile, pdb_fuc, tm):
...
def get_template_feat(pdbfile, pdb_fuc, pc, ag_ab, scaler):
    topk_tm = topk_tm_search(pdbfile, pc)
    topk_template_feat = []
    for tm_pc, tm_score in topk_tm:
        with open(f'{args.tm_library}/TM_library_feat/{tm_pc}.pkl', 'rb') as fd:
            tm_pc_feat = pickle.load(fd)
        align_label, align_feat = AlignmentMap(pdbfile, pdb_fuc, tm_pc, tm_pc_feat, scaler)
        rotation_pdbfile = rotation_structure(pdbfile, pdb_fuc, tm_pc)
